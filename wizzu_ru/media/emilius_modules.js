...
(function () {
  var COOKIE_NAME = "current_shop";
  var BARAKAT = "barakat-voda";
  var FERMER = "fermerskaya-lavka";

  // Маппинг ID коллекций магазинов (корневые + все подкатегории)
  var SHOP_COLLECTION_IDS = {
    "barakat-voda": [
      56465353, // корневая
      56465361,
      56465369,
  ...
  // Определяем магазин по URL (страницы коллекций)
  function detectShopByPath() {
    var path = window.location.pathname || "";
    if (path.indexOf("barakat-voda") !== -1) return BARAKAT;
    if (path.indexOf("fermerskaya-lavka") !== -1) return FERMER;
    return null;
  }

  // Определяем магазин по ID коллекции
  function detectShopByCollectionId(id) {
    id = Number(id);
    if (!id) return null;
    if (SHOP_COLLECTION_IDS[BARAKAT].indexOf(id) !== -1) return BARAKAT;
    if (SHOP_COLLECTION_IDS[FERMER].indexOf(id) !== -1) return FERMER;
    return null;
  }

  // Определяем магазин через AJAX к JSON товара
  async function detectShopFromProductAPI() {
    try {
      var path = window.location.pathname;
      if (path.indexOf("/product/") === -1) return null;

      var response = await fetch(path + ".json");
      if (!response.ok) return null;

      var data = await response.json();
      var product = data.product || data;

      // Основной способ — canonical коллекция товара
      var shop = detectShopByCollectionId(product.canonical_url_collection_id);
      if (shop) return shop;

      // Фолбэк — если в JSON есть список ID коллекций
      var ids = product.collection_ids || [];
      for (var i = 0; i < ids.length; i++) {
        shop = detectShopByCollectionId(ids[i]);
        if (shop) return shop;
      }

      return null;
    } catch (err) {
      console.warn("[ShopContext] Ошибка определения магазина через API:", err);
      return null;
    }
  }

  // Кэш в sessionStorage, чтобы не дёргать JSON повторно на том же товаре
  function getCacheForPath(path) {
    try {
      return sessionStorage.getItem("shop_cache_" + path);
    } catch (_) {
      return null;
    }
  }
  function setCacheForPath(path, shop) {
    try {
      sessionStorage.setItem("shop_cache_" + path, shop);
    } catch (_) {}
  }

  // Читаем cookie
  function getCookieShop() {
    var match = document.cookie.match(
      new RegExp("(?:^|; )" + COOKIE_NAME + "=([^;]*)"),
    );
    if (!match) return null;
    var value = decodeURIComponent(match[1]);
    if (value === BARAKAT || value === FERMER) return value;
    return null;
  }

  // Записываем cookie
  function setCookieShop(shop) {
    document.cookie =
      COOKIE_NAME +
      "=" +
  ...
  // Замена логотипов
  function applyLogo(shop) {
    var logos = document.querySelectorAll("[data-shop-logo]");
    Array.prototype.forEach.call(logos, function (img) {
      var attr = shop === BARAKAT ? "data-barakat" : "data-fermerskaya";
      var src = img.getAttribute(attr);
      if (src) img.src = src;
    });
  }

  // Замена title в модалке
  function applyModal(popup, shop) {
    popup = popup || document.getElementById("popup-message-warning");
    shop = shop || currentShop;
    if (!popup || !shop) return;

    var titleAttr =
      shop === BARAKAT ? "data-title-barakat" : "data-title-fermerskaya";
    var title = popup.getAttribute(titleAttr);
    var titleEl = popup.querySelector(".modal__title");
    if (titleEl && title) titleEl.textContent = title;
  }

  // Общее применение состояния
  function applyShop(shop) {
    if (!shop) return;
    document.documentElement.classList.add("shop-" + shop);
    applyLogo(shop);
    applyModal(null, shop);
  }

  async function init() {
    var path = window.location.pathname;

    // 1. Прямое определение по URL (страницы коллекций)
    var urlShop = detectShopByPath();
    if (urlShop) {
      setCookieShop(urlShop);
      currentShop = urlShop;
      applyShop(currentShop);
      return;
    }

    // 2. Страница товара — определяем через canonical_url_collection_id
    if (path.indexOf("/product/") !== -1) {
      var cached = getCacheForPath(path);
      if (cached === BARAKAT || cached === FERMER) {
        currentShop = cached;
      } else {
        var apiShop = await detectShopFromProductAPI();
        if (apiShop) {
          setCacheForPath(path, apiShop);
          currentShop = apiShop;
        }
      }

      if (currentShop) {
        setCookieShop(currentShop);
        applyShop(currentShop);
        return;
      }
    }

    // 3. Всё остальное — читаем cookie
    currentShop = getCookieShop();
    applyShop(currentShop);
  }
...
window.EM_Module.Modal = class {
  ...
  onClick(e) {
    ...
    if (
      ...
      e.target.classList.contains("icon_close") ||
      (e.target.nodeName == "svg" &&
        e.target.parentElement.classList.contains("modal-close"))
    ) {
      if (
        e.target.getAttribute("data-close-popup") &&
        e.target.getAttribute("data-close-popup") === this.modal.id
      ) {
        this.close();
        return;
      }
  ...
  open() {
    if (
      (this.modal.classList.contains("popup") ||
        this.modal.getAttribute("data-lock-page") === "true") &&
...
window.EM_Module.props = {
  ...
  types: {
    limit: 88291585, // макс. кол-вао
    pack: 88291713, // размер пачки,
    unit: 113430513, // единица измерения,
    gram: 121232513, // граммовка
  },
    ...
    for (const key in product.properties) {
      if (product.properties[key].permalink == "limit") {
        return {
          isAccept: true,
          quantity:
            product.properties[key].characteristics[0]?.title ??
  ...
  parseFormattedPrice(formattedPrice, options = Shop.money.options) {
    ...
    let str = formattedPrice
      .replace(options.unit, "")
      .replace(/&#8381/, "")
      .trim();
    ...
    if (options.format) {
      str = str
        .replace(options.format.replace("%n", "").replace("%u", ""), "")
        .trim();
    }
...
(() => {
  var timerId = setInterval(() => {
    if (
      ...
      typeof window?.EM_Module?.Loader !== "undefined"
    ) {
      clearInterval(timerId);

      document.dispatchEvent(
        new CustomEvent("em_module:ready", {
          cancelable: true,
        }),
      );
...
(() => {
  ...
  var timerId = null,
    $btnCart = EM_Module.isMobile
      ? $("[data-btn-mob-cart]:first")
      : $("[data-btn-cart]:first");
  ...
  function changeBtnAdd(cart) {
    // !Убрали стоимость доставки до двери
    // const positions_count = Cart.order.getItemByID(1823943729) === undefined ? cart.positions_count : cart.positions_count - 1;
    ...
    $btnCart
      .find("[data-em-cart-price]")
      .text(Shop.money.format(cart.total_price));
  }
  ...
  function checkProductInCart(order_lines) {
    ...
    if (ids.length > 0) {
      // const newIds = window.location.pathname.includes("/new_order") ? ids.filter(id => id != 1823943729) : ids;
      Cart.delete({
        items: ids,
      });
    }
  ...
  function changeQuantity(quantityBtn) {
    const isMinus = quantityBtn.getAttribute("data-btn-plus") === null,
      ...
      vairantId = Number(
        quantityBtn.parentElement.getAttribute("data-product-add"),
      ),
      error = input
        .closest("[data-variant-id]")
        ?.querySelector(".product__message-error");
    ...
    const unit = input.getAttribute("data-product-unit"),
      ...
      maxQuantityDefault = Number(
        input.getAttribute("data-max-qantity-default"),
      ),
      maxQuantityCustom = Number(input.getAttribute("data-max-qantity")),
      newQuantity = quantity + (isMinus ? -pack : pack);

    let maxQuantity =
      maxQuantityCustom < maxQuantityDefault
        ? maxQuantityCustom
        : maxQuantityDefault;

    if (
      newQuantity <= 0 ||
      (quantity > maxQuantity && quantity - (isMinus ? -pack : pack) <= 0)
    ) {
      ...
      if (quantity > maxQuantity) {
        ...
        input.value =
          quantity - (isMinus ? -pack : pack) + (unit ? " " + unit : "");
      }
    } else {
      if (error) $(error).fadeOut(150);
      input.value =
        newQuantity.toFixed(1).replace(/\.0$/, "") + (unit ? " " + unit : "");
  ...
  function initCart() {
    ...
    document.addEventListener("click", function (e) {
      if (e.target.getAttribute("data-product-add")) {
        ...
        e.target.parentElement.getAttribute("data-product-add")
      ) {
        onClickBtnAdd(e.target.parentElement);
      }
    });
...
$(document).ready(() => {
  function EMmaskPhone() {
    function formatInternationalPhone(formatted, digits) {
      ...
      formatted +=
        ...
        "(" +
        rest.substring(0, Math.min(2, rest.length));
      if (rest.length > 2) {
    ...
    function formatRussianPhone(formatted, digits) {
      ...
      formatted +=
        digits[0] + "(" + rest.substring(0, Math.min(3, rest.length));
      if (rest.length > 3) {
    ...
    document
      .querySelectorAll(
        "#shipping_address_phone, #client_phone, [data-input-phone], #client_field_38536811",
      )
      .forEach((input) => {
  ...
  if (
    pathname.includes("/new_order") ||
    pathname.includes("/contacts/new") ||
    pathname.includes("/client_account/contacts")
  ) {
    EMmaskPhone();
  }
  if (pathname.includes("/client_account/orders")) {
    ...
    if (navigator.userAgent.match(/Android/i)) {
      document.location.href =
        "https://play.google.com/store/apps/details?id=com.mycompany.wizzu";
    } else if (navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
      document.location.href =
        "https://apps.apple.com/ru/app/wizzu/id6747611030";
    }
  }
});

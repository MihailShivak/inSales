...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const isMaskToken = (value) => typeof value === "string" && /^%[a-z0-9_]+%$/i.test(value.trim());
    const isMessagingHref = (href) => /^tel:/i.test(href) || href.includes("wa.me") || href.includes("t.me");
    ...
    function resolveContacts(payload) {
        ...
        keys.forEach(function(key) {
            ...
            if (isMaskToken(str) && str.trim().toLowerCase() === `%${key.toLowerCase()}%`) {
                resolved[key] = def !== undefined && def !== null ? String(def) : "";
                return;
            }
            resolved[key] = str;
    ...
    function patchMessagingPhoneDigits(href, oldDigits, newDigits) {
        if (!oldDigits || !newDigits || oldDigits === newDigits || !isMessagingHref(href)) return href;
     ...
     */
    function findInstantForWallClockInZone(timeZone, year, month, day, hour, minute) {
        ...
        for (let t = start; t <= end; t += 60 * 1000) {
            ...
            if (y === year && mo === month && da === day && h === hour && mi === minute) {
                return d;
            }
        }
        return null;
    ...
    function updateContactStoreHours(root, from, to) {
        ...
        const startInst = findInstantForWallClockInZone(storeTz, y, m, d, fromHm.hour, fromHm.minute);
        const endInst = findInstantForWallClockInZone(storeTz, y, m, d, toHm.hour, toHm.minute);
    ...
    function applyStoreYandexMap(defaults, resolved) {
        const address = resolved.address !== "%address%" ? resolved.address : defaults.address;
    ...
    function initContactsPage() {
        ...
        replaceLegacyDefaultText(root, "data-contact-phone", defaults.phone, resolved.phone);
        replaceLegacyDefaultText(root, "data-contact-email", defaults.email, resolved.email);
        // replaceLegacyDefaultText(root, defaults.city, resolved.city);
        replaceLegacyDefaultText(root, "data-contact-address", defaults.address, resolved.address);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const $headings = $("[data-article-content]:first").find("h2, h3, h4, h5, h6");

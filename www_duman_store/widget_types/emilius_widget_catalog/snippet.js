...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const initialPage = Number.isFinite(pageFromURL) && pageFromURL > 0 ?
        pageFromURL :
    ...
    function reorderProductsBySubdomain(arr) {
        ...
        switch (window.location.hostname) {
            case "spb.duman.store":          swapAt(0, 1);                                   break;              // [1,0,2,3]
            case "nn.duman.store":           swapAt(0, 2);                                   break;              // [2,1,0,3]
            case "krasnoyarsk.duman.store":  swapAt(1, 2);                                   break;              // [0,2,1,3]
            case "ekaterinburg.duman.store": swapAt(0, 3);                                   break;              // [3,1,2,0]
            case "kazan.duman.store":        swapAt(1, 3);                                   break;              // [0,3,2,1]
            case "chelyabinsk.duman.store":  swapAt(2, 3);                                   break;              // [0,1,3,2]
            case "samara.duman.store":       swapAt(0, 1); swapAt(2, 3);                     break;              // [1,0,3,2]
            case "omsk.duman.store":         swapAt(0, 2); swapAt(1, 3);                     break;              // [2,3,0,1]
            case "rostov.duman.store":       swapAt(0, 3); swapAt(1, 2);                     break;              // [3,2,1,0]
            case "ufa.duman.store":          swapAt(0, 1); swapAt(0, 2);                     break;              // [2,0,1,3]
            case "voronezh.duman.store":     swapAt(0, 2); swapAt(0, 1);                     break;              // [1,2,0,3]
            case "perm.duman.store":         swapAt(0, 3); swapAt(0, 1);                     break;              // [1,3,2,0]
            case "kemerovo.duman.store":     swapAt(0, 3); swapAt(0, 2);                     break;              // [2,1,3,0]
            case "tolyatti.duman.store":     swapAt(0, 3); swapAt(1, 2); swapAt(0, 1);       break;              // [2,3,1,0]
            case "moscow.duman.store":       swapAt(0, 3); swapAt(1, 3); swapAt(2, 3);       break;              // [3,0,1,2]
        }
    ...
    function drawProduct(products) {
        ...
        const isPriceSort = currentOrder === "price" || currentOrder === "descending_price";
        const orderedProducts = isPriceSort ? products : reorderProductsBySubdomain(products);

        let html = "";

        for (let i = 0; i < orderedProducts.length; i++) {
            const itemHtml = Template.render(mapProductToTemplate(orderedProducts[i], i), "catalog-item");
            if (itemHtml) {
                html += itemHtml.trim();
            }
        }
    ...
    async function fetchCatalog(data = {}) {
        ...
        const requestData = {
            page: filters?.currentPage || initialPage || 1,
        ...
        try {
            ...
            if (!response) {
                console.warn("[New.Catalog]", "Пустой ответ API каталога");
                return null;
            }

            return response;
        } catch (error) {
            console.warn("[New.Catalog]", "Ошибка выполнения запроса каталога:", error);
            return null;
        } finally {
            isFetching = false;
        }
    ...
    function handlePaginationPageSwitch(page) {
        ...
        if (!Number.isFinite(newPage) || newPage < 1 || newPage === filters.currentPage) return;
    ...
    async function renderCatalog(clear = false) {
        ...
        if (!response) {
            if (clear) {
                ...
                $catalogState.text("Не удалось загрузить товары. Попробуйте обновить страницу.").removeAttr("hidden");
            } else {
                loaders.loader.visibleLoader(false);
            }
            return;
    ...
    function initCatalog() {
        ...
        if ($paginationBody.length && typeof window.EM_Module.Pagination === "function") {
    ...
    function setActiveMenu() {
        $catalog.find(`.catalog__navigation .catalog__navigation-item-link[href='${window.location.pathname}']:first`).addClass("_active");
    }

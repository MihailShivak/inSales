...
window.EM_Module.Filters = class {
    ...
    _getOptionsFilters() {
        ...
        const $fitlers = this.isMobStyle ? this.$filtresMob : this.$filtres;
        const className = (this.isMobStyle ? ".mob-popup__group" : ".filters__group") + "[data-filter-id]";
    ...
    getNameSort(value) {
        return this.modeCatalog ? this._getNameSortCatalog(value) : this._getNameSortWishlist(value);
    }
    ...
    async readingFilters() {
        ...
        if (
            ...
            const newFitlers = await this.fetch(
                formData,
                "/filters",
                this.typePage
            );
...
window.EM_Module.Colors = class {
    ...
    async draw(variantIds, ignoreSize=true) {
        ...
        const formData = new FormData();
        formData.append("flow_id", "62b2caa5-7874-40a9-b7f2-1b9561fe9e83");
        formData.append("variant_id", variantIds.at(-1) !== "," ? variantIds : variantIds.substring(0, variantIds.length - 1));
...
window.EM_Module.Badges = {
    ...
    getPropertyBadges: function(properties, characteristics, isFindType=false) {
        let badgeProp;
        ...
        for (const characteristic of characteristics) {
            if (characteristic.property_id == badgeProp.id) {
                badges.push(
                    isFindType ? this.findBadge(characteristic.title) : characteristic.title
                );
            }
        }
        return badges;
    ...
    findBadge: function(titleBadge) {
        ...
        return (
            this.badges.find(badge => badge.title.toLowerCase() === lowerTitle) ?? { title: titleBadge }
        );
    },

    renderBadgeHTML: (title, cssString) => {
        return !title ? "" : `<span class="products-item__badge" ${cssString ?? ""}>${title}</span>`;
    },
    ...
    getBadges: function(product) {
        const badges = this.getPropertyBadges(product.properties, product.characteristics, true);
    ...
    // Получить стили
    _getCSS: (css) => {
        return css.color || css.background ? `style="color:${css.color};background-color:${css.background}";` : "";
    }
};
...
window.EM_Module.func = {
    ...
    getImage: function (image) {
        ...
        const baseUrl = originalUrl.replace(
            originalUrl.includes("static.insales-cdn.com") ? "static.insales-cdn.com" : "ibicecdn.com/dumanstore",
            "ibicecdn.com/dumanstore/%size%"
        );
    
        const getUrl = (size, ratio) => baseUrl.replace("%size%", `l60/${Math.floor(size * ratio)}`);
        const innerWidth = window.innerWidth;
    
        let size = 565;
        if (innerWidth < 480) {
            size = 400;
        } else if (innerWidth > 1920 || (innerWidth > 768 &&innerWidth <= 1023.98)) {
            size = 730;
    ...
    getImageForCatalog: function (image, x = 1) {
        ...
        const getUrl = (size) => baseUrl.replace("%size%", `l60/${Math.floor(size * x)}`);
        const innerWidth = window.innerWidth;
...
window.EM_Module.FormFeedback = class {
    constructor($form, productID, typeForm) {
        ...
        this.url = this.typeForm == this.typesForms.subscription ? 
    ...
    submit() {   
        ...
        const name = this.$form.find("input[name='name']:first").val(),
            ...
            $recaptcha = this.$form.find("textarea[name='g-recaptcha-response']:first");
        let errors = [];
    ...
    preventDoubleSubmit(isSave=false) {
        const lastSubmitTime = localStorage.getItem("lastFormPromoSubmit-" + this.typeForm);
        ...
        if (isSave) {
            localStorage.setItem("lastFormPromoSubmit-" + this.typeForm, now.toString());
        }
        return true;
    }
        ...
        if (!errors || !errors.length) {
            ...
            for (const err of errors) {
                ...
                if (textError.includes("roduct is not specified")) {
                    ...
                    message += "данный адрес электронной почты недействителен — на него невозможно доставить письмо с оповещением" + ", ";
                }
                else if (textError.includes("is available already")) {
                    message += "данный товар на момент получения запроса есть в наличии" + ", ";
                }
                else if (textError.includes("does not exist")) {
                    message += "данный товар не существует в системе, поэтому на него невозможно оформить подписку" + ", ";
                }
                else if (textError.includes("pecified customer already has a subscription to this product")) {
    ...
    showSuccess() {
        ...
        this.$form.find("[data-em-message]").attr("hidden", false)
            ...
            .find("span:first").text(
                this.typeForm == this.typesForms.subscription ? "Данные сохранены" : "Ваш запрос успешно отправлен"
            );
    }
    ...
    showErrors(errors) {
        ...
        for (const error of errors) {
            ...
            const $parent = this.$form.find(`.input[name="${error.type}"]`).closest(".input-group"),
            ...
            if ($inputError.length) {
                ...
                $parent.addClass("input-error").append(`<span class="form-mess__error">${error.text}</span>`);
            }
        }
    ...
    class WishlistAPI {
        ...
        async init(renderIcon = true) {
            ...
            if (this.clientID) {
                ...
                EventBus.subscribe(this.nameEventUpdate, this.updateWishlist.bind(this));
        ...
        updateWishlist(data) {
            ...
            for (const productID of data.items) {
                document.querySelectorAll(`[data-em-favorites-trigger="${productID}"]`).forEach(icon => {
        ...
        async setWishlist() {
            ...
            if (this.wishlistIDs.length > this.maxItems) {
                const delItems = this.wishlistIDs.splice(this.maxItems - 1, this.wishlistIDs.length - this.maxItems);
        ...
        async add(productID, variantID) {
            ...
            if (this.wishlistIDs.length + 1 > this.maxItems) {
                const delItems = this.wishlistIDs.splice(this.maxItems - 2, this.wishlistIDs.length - this.maxItems + 1);
        ...
        checkOverflowWishilst() {
            ...
            const delItems = this.wishlistIDs.splice(this.maxItems, this.wishlistIDs.length - this.maxItems);
        ...
        async fetch(type, formData = new FormData()) {
            ...
            if (!response) {
                console.warn(this._name, "Ошибка получения списка избранного:", response);
            }
            return response;
        }
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleMediaChange(event) {
        ...
        document.documentElement.style.setProperty('--notice-basic-height', (height) + "px");

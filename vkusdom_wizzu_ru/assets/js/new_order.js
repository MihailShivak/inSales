document.addEventListener("DOMContentLoaded", function() {
    ...
    const ProductDeliveryID = 1823943729;
    ...
    */
    ...
    function setDoorPrice(isDoor) {
        const 
            door = Cart.order.getItemByID(ProductDeliveryID),
        ...
        if (isDoor && door !== undefined) {
            ...
            if (actionDoor == "del") {
                ...
                if (newTotalPrice && !isNaN(newTotalPrice)) {
                    $totalPrice.text(
                        Shop.money.format(newTotalPrice < 30 ? 0 : newTotalPrice - 30)
                    );
                }
            }
    ...
    function cheangeProductDoor(isDoor) {
        ...
        // const isDoorInOrder = Cart.order.getItemByID(ProductDeliveryID) !== undefined;
        // if (isDoor) {
        //     if (!isDoorInOrder) {
        //         actionDoor = "set";
        //         Cart.set({
    ...
    function setHTMLBlocks() {
        ...
        $orderForm.find(".co-input--product-unavailable > input.co-input-field").val(
            productUnavailable == "change-on-any" ? "Позвонить для замены" : "Заменить самостоятельно" 
                // productUnavailable == "change-in-order" ? "Заменить на товар из заказа" : "Не заменять" 
        );
    ...
    $(document).on("selected:insales:payment", function(event) {
        ...
        if (event.originalEvent.detail.id === 12038801) {
            $input.show(250);
    ...
    $orderForm.find(
        ...
        "input[name='shipping_address[house]']," + 
        "#em-address"
    )

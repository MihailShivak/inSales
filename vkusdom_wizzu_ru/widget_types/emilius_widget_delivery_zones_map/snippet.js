class MapZones {
    ...
    addDistrictPolygon(shop) {
        try {
            ...
            const features = rings.map(ring => new YMapFeature({
                geometry: {
                    type: 'Polygon',
                    coordinates: [ring]
                },
...
class DeliveryZoneSelector {
    ...
    checkAndUpdateLastVisit(lastVisitTime, isSetTime = false) {
        const HALF_DAY_MS = 60 * 60 * 1000; // 1 час
        const now = Date.now();

        if (!lastVisitTime || lastVisitTime === "false") {
            if (isSetTime) localStorage.setItem("lastVisitTime", String(now));
            return true;
        }

        const lastTime = Number(lastVisitTime);
